...
_ENVNAME = 'PORTTESTER_TEST_ZFS_POOL'
...
def zfs_pool() -> Path:
    ...
    if pool is None:
        pytest.skip('no test ZFS pool defined with PORTTESTER_TEST_ZFS_POOL environment variable')
    else:
        return Path(pool)
...
async def zfs(zfs_pool) -> AsyncGenerator[Any, ZFS]:
    ...
    zfs = ZFS(zfs_pool).get_child('porttester_test')
